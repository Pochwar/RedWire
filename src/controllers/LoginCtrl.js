                ...
                if (user.mailValid) {
                    ...
                    encrypt.compare(req.body.password, user.password, (err, resp) => {
                        if (resp) {
                            ...
                            res.status(400).json({ error: "loginError", msg: res.__('LOGIN_ERROR'), });
                        }
                    });
                } else {
                    winston.info(`email not active`);
                    res.status(400).json({ error: "unconfirmedMail", msg: res.__('UNCONFIRMED_MAIL'), });
                }
